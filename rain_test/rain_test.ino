...
int SENSOR_POWER_PIN= 5; // Use TIP142T to power the sensor (High will shout it down Low will turn it on)
int MOTOR_PIN = 8; // Use the onboard Uno LED
int WET_PIN = 6; // Use the onboard Uno LED
int DRY_PIN = 7; // Use the onboard Uno LED

int isRaningPin = 3;  // This is our input pin
int rainingAnalogPin = 0;  // This is our input pin for analog data
...
void loop() {
  ...
  if(start_time+minute_in_mill>=millis()) {
    // fire up rain sensor
    digitalWrite(SENSOR_POWER_PIN,HIGH); // turn the sensor off  
    delay(1);// for sensor pin to load before i take sample
